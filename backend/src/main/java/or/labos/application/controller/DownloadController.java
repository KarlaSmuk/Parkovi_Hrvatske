...
public class DownloadController {
    ...
    /*@CrossOrigin
    @PostMapping("/search/downloadAsJSON")
    public ResponseEntity<List<ParkToFileDto>> json(@RequestBody List<Map<String, Object>> requestData) {
        ...
        requestData.forEach(park -> {
            ...
            if (!Objects.isNull(park.get("peakName"))) {
                Map<String, Object> vrhMap = new HashMap<>();
    ...
    public ResponseEntity<List<ParkToFileDto>> json(@RequestBody List<Map<String, Object>> requestData) {
        ...
        requestData.forEach(park -> {
            ParkToFileDto existingPark = parkToFileDtos.stream()
                    .filter(p -> p.getPark().equals((String) park.get("parkName")))
                    .findFirst()
                    .orElse(null);

            // Check if the park already exists
            //ParkToFileDto existingPark = findExistingPark(parkToFileDtos, (String) park.get("parkName"));
